...
public class RareEngieRCTameProcedure {
			...
			if ((sourceentity instanceof LivingEntity _livEnt ? _livEnt.getMainHandItem() : ItemStack.EMPTY).getItem() == Items.COOKIE) {
				if (Math.random() >= 0.8) {
					if (world instanceof ServerLevel _level) {
						Entity entityToSpawn = new RareEngieSharkoTamedEntity(AllAboutEngieThrowbackModEntities.RARE_ENGIE_SHARKO_TAMED.get(), _level);
